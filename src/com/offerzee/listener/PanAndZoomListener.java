 ...
 * 
 */
public abstract class PanAndZoomListener implements OnTouchListener {
	...
	static final int DOUBLE_CLICK_ZOOM_SCALE = 4;
	int mode = NONE;
	...
	private boolean showExtraWidgets = true;
	public abstract void showExtraWidgets(boolean showExtraWidgets);
		...
		switch (event.getAction() & MotionEvent.ACTION_MASK) {
			case MotionEvent.ACTION_DOWN :
				...
				if (thisTime - lastTouchTime < 250) {
					lastTouchTime = -1;
					if (panZoomCalculator.getCurrentZoom() >= DOUBLE_CLICK_ZOOM_SCALE) {
						panZoomCalculator.reset();
						showExtraWidgets = true;
						showExtraWidgets(showExtraWidgets);
					} else {
						PointF clickPoint = new PointF();
						clickPoint.set(event.getRawX(), event.getRawY());
						panZoomCalculator.doZoom(DOUBLE_CLICK_ZOOM_SCALE,
								clickPoint);
					...
					showExtraWidgets = showExtraWidgets == true ? false : true;
					showExtraWidgets(showExtraWidgets);
			...
			case MotionEvent.ACTION_MOVE :
				if (mode == DRAG) {
					panZoomCalculator.doPan(event.getX() - start.x,
							event.getY() - start.y);
					start.set(event.getX(), event.getY());
				} else if (mode == ZOOM) {
